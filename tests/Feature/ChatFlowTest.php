...
class ChatFlowTest extends TestCase
{
    ...
    public function test_user_can_create_conversation_with_admin()
    {
        ...
        $this->assertDatabaseHas('conversations', [
            'id' => $conversation->id,
            'user1_id' => min($this->regularUser->id, $this->adminUser->id),
            'user2_id' => max($this->regularUser->id, $this->adminUser->id),
        ]);
    ...
    public function test_user_can_get_unread_count()
    {
        ...
        // Get unread count for user (direct query to avoid Redis)
        $unreadCount = Message::where('is_read', false)
            ->whereHas('conversation', function ($query) {
                $query->where('user1_id', $this->regularUser->id)
                    ->orWhere('user2_id', $this->regularUser->id);
            })
            ->where('sender_id', '!=', $this->regularUser->id)
            ->count();

        // Should be 3 (from admin)
    ...
    public function test_user_can_get_their_conversations()
    {
        ...
        // Get conversations for user (direct query to avoid Redis)
        $conversations = Conversation::where('user1_id', $this->regularUser->id)
            ->orWhere('user2_id', $this->regularUser->id)
            ->with('messages')
            ->get();

        $this->assertCount(1, $conversations);
        $this->assertEquals($conversation->id, $conversations->first()->id);
    }
