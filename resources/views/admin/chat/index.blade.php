...
@section('content')
    ...
    <style>
        ...
        .message.sent .message-content:has(.message-file) {
        ...
        .message.received .message-content:has(.message-file) {
            background: transparent;
            border: none;
            padding: 0;
            border-radius: 0;
    ...
    <script>
        ...
        function displayConversations(conversations) {
            ...
            conversations.forEach(conv => {
                ...
                const lastMessageText = conv.last_message && conv.last_message.content ? conv.last_message.content :
                    (conv.last_message && conv.last_message.file_name ? conv.last_message.file_name :
                        'لا توجد رسائل');
        ...
        function displayChat(data) {
            ...
            const otherUser = data.conversation.user1_id === {{ auth()->id() }} ? data.conversation.user2 : data
                .conversation.user1;
    ...
    `;
        ...
        function createMessageHTML(message) {
            ...
            if (message.type === 'image') {
                const isBlob = message.file_path && message.file_path.startsWith('blob:');
                const imagePath = isBlob
                    ? message.file_path
                    : '/admin/chat/api/chat/file/' + message.file_path;
                const onClickAction = isBlob ? '' : `onclick="window.open('${imagePath}', '_blank')"`;
                const downloadAction = isBlob ? '' : `onclick="window.open('${imagePath}', '_blank')"`;
                messageContent = `<div class="message-image" style="position: relative; display: inline-block;">
                <img src="${imagePath}" alt="${message.file_name || 'صورة'}"
                     ${onClickAction}
                     style="max-width: 250px; max-height: 300px; border-radius: 8px; cursor: pointer; display: block;">
                <div style="position: absolute; bottom: 8px; right: 8px; background: rgba(0,0,0,0.6); color: white; padding: 6px 10px; border-radius: 4px; font-size: 12px; cursor: pointer;" ${downloadAction}>
                    <i class="fa-solid fa-download"></i>
                </div>
            </div>`;
            } else if (message.type === 'file') {
                const filePath = message.file_path && message.file_path.startsWith('blob:')
                    ? message.file_path
                    : '/admin/chat/api/chat/file/' + message.file_path;
                const downloadHref = message.file_path && message.file_path.startsWith('blob:')
                    ? '#'
                    : filePath;
                messageContent = `<div class="message-file">
                <i class="fa-solid fa-file" style="font-size: 24px; color: #007bff;"></i>
                <div style="flex: 1; overflow: hidden;">
                    <div style="font-size: 0.9em; word-break: break-word; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${message.file_name || 'ملف'}</div>
                </div>
                <a href="${downloadHref}" download="${message.file_name}" style="text-decoration: none; color: #007bff; cursor: pointer; padding: 6px 10px; flex-shrink: 0;">
            ...
            if (isSent) {
                if (message.is_read) {
                    readReceipt =
                        '<i class="fa-solid fa-check-double" style="color: #4FC3F7; font-size: 12px; margin-right: 4px;"></i>';
                } else {
                    readReceipt =
        ...
        function sendMessage() {
            ...
            if (files && files.length > 0) {
                // Send multiple files with optimistic UI
                for (let i = 0; i < files.length; i++) {
                    ...
                    // Show optimistic file message immediately
                    const tempMessage = {
                        id: tempId,
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                if (tempElement) {
                                    ...
                                    // Re-render the message content with real file path
                                    const msgDiv = tempElement.querySelector('.message-content');
                                    if (msgDiv) {
                                        // Build fresh innerHTML using the real message data
                                        const realMsg = {...data.data, sender_id: {{ auth()->id() }}};
                                        let inner = '';
                                        if (realMsg.type === 'image') {
                                            const p = '/admin/chat/api/chat/file/' + realMsg.file_path;
                                            inner = `<div class="message-image" style="position: relative; display: inline-block;">
                                                <img src="${p}" alt="${realMsg.file_name||'صورة'}" onclick="window.open('${p}','_blank')" style="max-width:250px;max-height:300px;border-radius:8px;cursor:pointer;display:block;">
                                                <div style="position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.6);color:white;padding:6px 10px;border-radius:4px;font-size:12px;cursor:pointer;" onclick="window.open('${p}','_blank')"><i class="fa-solid fa-download"></i></div>
                                            </div>`;
                                        } else if (realMsg.type === 'file') {
                                            const p = '/admin/chat/api/chat/file/' + realMsg.file_path;
                                            inner = `<div class="message-file">
                                                <i class="fa-solid fa-file" style="font-size:24px;color:#007bff;"></i>
                                                <div style="flex:1;overflow:hidden;"><div style="font-size:0.9em;word-break:break-word;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${realMsg.file_name||'ملف'}</div></div>
                                                <a href="${p}" download="${realMsg.file_name}" style="text-decoration:none;color:#007bff;cursor:pointer;padding:6px 10px;flex-shrink:0;"><i class="fa-solid fa-download" style="font-size:18px;"></i></a>
                                            </div>`;
                                        }
                                        if (inner) msgDiv.innerHTML = inner;
                                    }
        ...
        function handleFileSelect(event) {
            ...
            preview.style.cssText =
                'margin-top: 10px; padding: 10px; background: #f0f0f0; border-radius: 8px; display: flex; flex-wrap: wrap; gap: 10px;';

            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                const fileItem = document.createElement('div');
                fileItem.style.cssText =
                    'display: flex; align-items: center; gap: 8px; background: white; padding: 8px; border-radius: 6px;';
                ...
                fileName.style.cssText =
                    'max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 0.9em;';
